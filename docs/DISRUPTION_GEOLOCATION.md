...
> Status: **rail and bus-stop layers built and running locally** (2026-09-04); P1, the sentence
> parser, is deferred and evidence-gated (§11.1). This is the
...
- **Stop-level closures ship first, and they are the structured half of the bus picture**
  (*measured* 2026-09-04, reversing the earlier deferral). `/StopPoint/Mode/bus/Disruption`
  returns ~300 rows keyed by ATCO id, each with a name, a from/to window and free text — the
  same shape as the rail Tier 0, so no sentence is parsed. What was missing was only the
  id → coordinate join, and it did not need the feared 19,000-stop bake: `GET /StopPoint/{ids}`
  resolves 20 ids per call and a stop's position never changes, so ids are resolved once and
  cached forever in `data/bus-stops/gazetteer.json`. The first run resolved **261/261 poles in
  14 calls**; a rerun costs zero. Every entry also carries the routes serving the stop, which
  is the join key for the search-scoped view, and a `towards` direction for 88% of poles, so a
  pin says which side of the road. Median distance from a closed pole to the learned polyline
  of a route serving it is **5 m** (60-stop sample; the 5 outliers past 100 m are all rural
  Bromley routes whose learned shape is coarse, not bad stop data).
- **Route-level bus notices carry no location at all** (*measured*: 201 *Special Service*
  statuses, `affectedStops` 0 on every one, and the prose names streets — *"EVERSHOLT STREET,
  NW1"*, *"COMMERCIAL ROAD E1"* — not stops). A whole-route wash plus the full text is the
  honest ceiling; a parser would need a street gazetteer, not a station one.
...
| **Bus stops (built 2026-09-04)** | ATCO gazetteer + `GET /api/bus-stop-closures` + a default-off overlay drawing every stop closed **right now** | 1.5 days | 304 feed rows reconcile to 254 pins with an empty set difference both ways; 254/254 verified against the raw feed and TfL's own StopPoint answer |
...
| 2026-09-04 | Rail layer reviewed on the running map. It hatched the whole Waterloo & City line closed on a Friday morning: live and planned were split by TfL's *category*, so an `Information` notice ("no service Saturdays and Sundays") was neither `P` nor `R` and counted as live. The mirror was worse — a `PlannedWork` closure that had begun kept category `P` and would have sat in the default-off planned source, showing nothing on the morning a line is genuinely shut. Both now key off `n` (TfL's own `isNow`); category only words the window. The fixture that hid it said `c: 'R'` with `n: 0`. |
| 2026-09-04 | Minor-delay washes were nearly invisible — dash units are multiples of line width and the dash was shorter than its gap at 0.45 opacity. Now longer than its gap at 0.6. The Lines tab's per-line Service strip was removed at the owner's request (four truncated, unclickable rows that pushed the overlay toggles off the panel); the line-row pip already carries the same wording, and one row survives for feed outages so a dead backend never reads as an all-clear. |
| 2026-09-04 | **Bus stop closures shipped** (§9): gazetteer, route, overlay. Review caught the stop-resolution path bypassing the shared TfL rate budget, a missing `type` being defaulted to "Closure" (claiming what the row does not say), and an `async` hook's rejection escaping the route-shape notifier unlogged. TfL's descriptions carry 909 *literal* backslash-n sequences and zero real control characters. |
